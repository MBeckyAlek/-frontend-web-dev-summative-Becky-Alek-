...
const SELECTORS = {
    ...
    recordsCardsContainer: document.getElementById('records-cards-container') || null,
...
export const UI = {
    ...
    switchView: (viewName) => {
        ...
        // Clear form errors when switching away from form
        if (viewName !== 'form') {
            UI.clearFormErrors();
        }
        ...
        document.getElementById('skip-to-content').href = `#${headingId}`;
    },
    ...
    openMobileMenu: () => {
        ...
        document.getElementById('mobile-menu-toggle').classList.add('active');
     ...
     */
    toggleMobileMenu: () => {
        if (SELECTORS.sidebar.classList.contains('open')) {
            UI.closeMobileMenu();
        } else {
            UI.openMobileMenu();
        }
    ...
    /**
     * Loads form with record data for editing
     * @param {Object|null} record - Record to edit or null for new record
     * @param {Array} categories - Available categories
     */
    loadForm: (record = null, categories = []) => {
        const form = SELECTORS.transactionForm;
        ...
        if (record) {
            // Reset form and then populate with record data
            form.reset();
        ...
        document.getElementById('advanced-check').textContent = '';
        UI.clearFormErrors();
    ...
    updateBudgetDisplay: (budgetInfo, settings) => {
        ...
        // Update progress bar
        const progressFillElement = document.getElementById('budget-progress-fill');
        if (progressFillElement) {
            progressFillElement.style.width = `${spentPercentage}%`;
            if (isOverBudget) {
                progressFillElement.style.background = 'var(--color-error)';
            } else {
                progressFillElement.style.background = 'var(--color-primary)';
            }
        }
    ...
    /**
     * Loads form with categories and optional record data
     * @param {Object|null} record - Record data to populate form (null for new record)
    ...
    loadForm: (record, categories) => {
        ...
        // Clear form if no record provided
        if (!record) {
